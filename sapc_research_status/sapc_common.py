# -*- coding: utf-8 -*-
"""
叠层铝固态电容器（SAPC）国内外研究现状专题综述 —— 通用样式与排版辅助模块
Common styling / layout helpers for the SAPC (Stacked Aluminium solid Polymer Capacitor)
"Research Status" review.

设计目标：
1. 自包含，不依赖其他报告包，便于独立构建与下载。
2. 提供中文标题/正文/公式/三线表/插图/评述框/自动目录(域)等排版能力。
...
def build_cover(doc, title_lines, subtitle_en, meta):
    ...
    run2 = p2.add_run("国 内 外 研 究 现 状 专 题")
    set_cn_font(run2, font_name="黑体", size_pt=16, bold=True)
    doc.add_paragraph()
    doc.add_paragraph()
...
def new_document():
    ...
    add_header_text(doc, "叠层铝固态电容器（SAPC）国内外研究现状")
