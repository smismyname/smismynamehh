# -*- coding: utf-8 -*-
"""
叠层铝固态电容器（SAPC）失效模式与失效机理国内外研究现状综述
通用样式与排版辅助模块 (self-contained, python-docx)

设计目标：
1. 自包含，不依赖其他包，便于独立构建与下载。
2. 提供中文标题/正文/公式/三线表/插图/自动目录(域)等排版能力。
3. 采用"章-序"图表编号（图 2-1、表 3-2 等），符合学位论文规范。
4. 图内统一英文/数字标注（沙箱无 CJK 字体），图题(中文)由 Word 渲染。
"""
import os
...
class Counters:
    ...
    def next_fig(self):
        self.fig += 1
        return "%d-%d" % (self.chapter, self.fig)

    def next_tab(self):
        self.tab += 1
        return "%d-%d" % (self.chapter, self.tab)

    def next_eq(self):
        self.eq += 1
        return "%d-%d" % (self.chapter, self.eq)
...
def add_quote(doc, text, label="评述"):
    """评述 / 重点框：左缩进、楷体、带左边框"""
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    ...
    p.paragraph_format.space_after = Pt(6)
    if label:
        r0 = p.add_run("【%s】" % label)
        set_cn_font(r0, font_name="黑体", size_pt=11.5, bold=True, color=RGBColor(0x2F, 0x54, 0x96))
...
def add_table(doc, headers, rows, caption=None, col_widths=None, font_size=9.5,
              header_size=9.5):
    """三线表：上下粗线、表头下细线。caption 自动按"章-序"编号。返回表号字符串。"""
    label = None
    if caption:
        label = COUNT.next_tab()
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p.paragraph_format.space_before = Pt(6)
        p.paragraph_format.space_after = Pt(2)
        p.paragraph_format.keep_with_next = True
        run = p.add_run("表 %s  %s" % (label, caption))
...
def add_table_caption_note(doc, text):
    """表/图下方的小字注释（来源、说明）"""
    p = doc.add_paragraph()
    ...
    run = p.add_run(text.strip())
    set_cn_font(run, font_name="楷体", size_pt=9)
    return p
...
# 插图
# ----------------------------------------------------------------------------
def add_figure(doc, img_path, caption, width_cm=14.5, note=None):
    """插图，按"章-序"编号。返回图号字符串。note 为图下来源/解读小字。"""
    ...
    label = COUNT.next_fig()
    cap = doc.add_paragraph()
    cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    cap.paragraph_format.space_after = Pt(2 if note else 8)
    crun = cap.add_run("图 %s  %s" % (label, caption))
    set_cn_font(crun, font_name="黑体", size_pt=10.5, bold=True)
    if note:
        np_ = doc.add_paragraph()
        np_.alignment = WD_ALIGN_PARAGRAPH.CENTER
        np_.paragraph_format.space_after = Pt(8)
        np_.paragraph_format.line_spacing = 1.1
        nrun = np_.add_run(note.strip())
        set_cn_font(nrun, font_name="楷体", size_pt=9)
    return label
...
# 列表项
# ----------------------------------------------------------------------------
def add_list_item(doc, text, size=12, indent=0.82):
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
...
def add_toc_field(doc):
    ...
    t = OxmlElement('w:t')
    t.text = "（请在 Word 中按 Ctrl+A 全选后按 F9 更新目录域以生成页码）"
    fldChar_end = OxmlElement('w:fldChar')
    fldChar_end.set(qn('w:fldCharType'), 'end')
    r = run._r
    r.append(fldChar)
...
def build_cover(doc, title_lines, subtitle_en, meta):
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("博 士 学 位 论 文")
    set_cn_font(run, font_name="黑体", size_pt=24, bold=True)
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("国 内 外 研 究 现 状 综 述")
    ...
    for line in title_lines:
        ...
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = p.add_run(line)
        set_cn_font(run, font_name="黑体", size_pt=19, bold=True)
    doc.add_paragraph()
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run(subtitle_en)
    set_cn_font(run, font_name="Times New Roman", size_pt=12.5, italic=True)
    for _ in range(6):
        doc.add_paragraph()
    for k, v in meta:
        p = doc.add_paragraph()
...
def new_document():
    ...
    add_header_text(doc, "叠层铝固态电容器（SAPC）失效模式与失效机理国内外研究现状")
